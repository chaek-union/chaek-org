...
import { parseSummary, getBookRoot, getFilePaths, getFilePathsWithChapters } from './summary-parser.js';
...
async function syncRepository(repoName: string, repoUrl: string): Promise<void> {
	...
	try {
		...
		try {
			...
			console.log(`Removing invalid directory and cloning ${repoName}...`);
...
async function buildSearchIndex(repoName: string, buildId: number): Promise<CompilationResult> {
	try {
		await buildEvents.emitLog(buildId, 'stdout', `Building search index for ${repoName}...`);
		...
		await buildEvents.emitLog(buildId, 'stdout', `Found ${filePathsWithChapters.length} files to index`);
		...
		await buildEvents.emitLog(buildId, 'stdout', `Successfully indexed ${validDocuments.length} documents`);
 ...
 */
async function buildPdf(repoName: string, buildId: number): Promise<{ success: boolean; pdfPath?: string; error?: string }> {
	try {
		await buildEvents.emitLog(buildId, 'stdout', `Building PDF for ${repoName}...`);
		...
		await buildEvents.emitLog(buildId, 'stdout', `Found ${filePaths.length} files to compile`);
		...
		for (const filePath of filePaths) {
			try {
				...
				while ((match = imageRegex.exec(content)) !== null) {
					...
					if (!downloadedImages.has(url)) {
						try {
							...
							if (asParam && asParam.match(/^(webp|png|jpg|jpeg|gif|svg)$/i)) {
							...
							const localPath = path.join(tmpAssetsDir, localFilename);

							// Download the image using curl with user-agent to bypass bot protection
							await buildEvents.emitLog(buildId, 'stdout', `Downloading image: ${url}`);
							...
							const fileStart = fileBuffer.toString('utf-8', 0, Math.min(200, fileBuffer.length)).trim();

							if (fileStart.startsWith('<!DOCTYPE') || fileStart.startsWith('<html')) {
								// Downloaded HTML instead of image (likely Cloudflare challenge)
								await fs.unlink(localPath);
								throw new Error('Downloaded HTML instead of image (bot protection detected)');
							}

							// For SVG files, verify they start with valid SVG content
							if (ext === '.svg') {
								if (!fileStart.startsWith('<?xml') && !fileStart.startsWith('<svg')) {
									// Invalid SVG file
									await fs.unlink(localPath);
									throw new Error('Downloaded invalid SVG file');
								}
							}
							...
							if (ext === '.webp') {
								...
								const pngPath = path.join(tmpAssetsDir, pngFilename);
								await buildEvents.emitLog(buildId, 'stdout', `Converting WebP to PNG: ${localFilename}`);
								await new Promise<void>((resolve, reject) => {
									const convert = child_process.spawn('convert', [localPath, pngPath]);
									convert.on('close', (code: number | null) => {
										if (code === 0) {
											resolve();
										} else {
							...
							downloadedImages.set(url, `assets/${finalFilename}`);
							imageCounter++;
						} catch (err) {
							console.error(`Failed to download image ${url}:`, err);
							await buildEvents.emitLog(buildId, 'stderr', `Failed to download image: ${url}`);
						}
					}
				}

				// Replace remote URLs with local paths or alt text
				content = content.replace(/!\[([^\]]*)\]\((https?:\/\/[^\)]+)\)/g, (_match: string, alt: string, url: string) => {
				...
				content = content.replace(/!\[([^\]]*)\]\((?!http|\/|assets\/)(.*?)\)/g, (match: string, alt: string, imgPath: string) => {
		...
		try {
			...
			const bookConfig = JSON.parse(await fs.readFile(bookJsonPath, 'utf-8'));
		...
		await new Promise<void>((resolve, reject) => {
			const pandoc = child_process.spawn('pandoc', [
				combinedMd,
				'-o', outputPdf,
				'--pdf-engine=xelatex',
				'--resource-path=.:assets',
				'-V', 'mainfont:KoPubWorldDotum Medium',
				'-V', 'mainfontoptions:BoldFont=KoPubWorldDotum Bold,ItalicFont=KoPubWorldDotum Light',
			...
			pandoc.stdout.on('data', (data: any) => {
				...
				for (const line of lines) {
					if (line.trim()) {
						logQueue = logQueue.then(() => buildEvents.emitLog(buildId, 'stdout', line));
					}
				}
			});
			...
			pandoc.stderr.on('data', (data: any) => {
				...
				for (const line of lines) {
					if (line.trim()) {
						logQueue = logQueue.then(() => buildEvents.emitLog(buildId, 'stderr', line));
					}
				}
			});
			...
			pandoc.on('close', async (code: number | null) => {
				...
				if (stdoutBuffer.trim()) {
					logQueue = logQueue.then(() => buildEvents.emitLog(buildId, 'stdout', stdoutBuffer));
				}
				if (stderrBuffer.trim()) {
					logQueue = logQueue.then(() => buildEvents.emitLog(buildId, 'stderr', stderrBuffer));
				}
				...
				try {
					...
					await buildEvents.emitLog(buildId, 'stdout', 'PDF generated successfully despite warnings');
					resolve();
				} catch {
...
export async function processBook(
	...
	try {
		...
		if (pdfResult.success) {
			await buildEvents.emitLog(buildId, 'stdout', `PDF built successfully: ${pdfResult.pdfPath}`);
		} else if (pdfResult.error) {
			await buildEvents.emitLog(buildId, 'stderr', `PDF build failed: ${pdfResult.error}`);
		}
		...
		console.log(`Build completed for ${repoName}: index=${indexResult.success}, pdf=${pdfResult.success}`);

		return {
			success: allSuccess,
			message: allSuccess ? `Build completed for ${repoName}` : `Build completed with errors for ${repoName}`,
			buildId
		};
	} catch (error) {
