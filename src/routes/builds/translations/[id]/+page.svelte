<script lang="ts">
	...
	const pageTitle = $derived(`Translation #${data.logId} - ${data.bookId} → ${data.targetLocale} - ${$t('app.title')}`);

	onMount(() => {
		eventSource = new EventSource(`/api/translations/${data.logId}/stream`);
